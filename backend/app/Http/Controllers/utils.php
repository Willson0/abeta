...
//                'parse_mode' => 'Markdown'
    ...
    static function transliterate ($text) {
        ...
        $text = str_replace($rus, $lat, $text);                  // Транслит
        $text = preg_replace('/\s+/', '_', $text);               // Пробелы → _
        $text = preg_replace('/[^A-Za-z0-9_]/', '', $text);      // Удалить всё лишнее
        $text = preg_replace('/_+/', '_', $text);                // Сдвоенные _ → один
        $text = trim($text, '_');                                // Убрать _ в начале/конце
        return $text;
    }
}

...
export default {
    ...
    methods: {
        ...
        async fetchProduct () {
            await fetch (config.backend + "product/" + this.$route.params["id"], {
                method: "GET",
                credentials: "include",
            }).then((response) => {
                ...
                this.stock_quantity = response.stock_quantity;
                ...
                response.images.forEach(img => {
                    fetch(config.backend + `storage?filepath=${img.image_url}`, {
                        credentials: "include",
                        method: "GET",
                    }).then(res => {
                        return res.blob();
                    }).then(res => {
                        let file = new File([res], "image.jpg", { type: res.type });
                ...
                response.deleted_at ? this.deleted = true : '';
                removeLoading();
            });
        },
        formatText () {
        ...
        async sendData() {
            ...
            await fetch (config.backend + `product/${this.product.id}`, {
                method: "POST",
                credentials: "include",
                body: formData,
            }).then((response) => {
                ...
                this.notify("Product was successfully update!")
            });
        },
        addimg (ev) {
        ...
        updImgs() {
            ...
            this.images.forEach((el, index) => {
                ...
                let url = null;
                if (el instanceof File) url = URL.createObjectURL(new Blob([el], {type: el.type}));
                else url = el;
        ...
        async deleteproduct () {
            await fetch (config.backend + `product/${this.product.id}`, {
                ...
                return this.$router.push({name: "productsAdmin"});
            });
        }
    },
    components: {
...
<template>
    ...
    <div class="admin_indexuser_popup_delete">
        <div>
            ...
            <h2>Are you sure you want to<br>delete this product</h2>
            <p>[{{product.id}}] {{product.name}}</p>
            <div>
                <button class="admin_indexuser_popup_delete_cancel" @click="togglePopup('admin_indexuser_popup_delete')">Cancel</button>
                <button class="admin_indexuser_popup_delete_delete" @click="deleteproduct()">Delete</button>
            </div>
        </div>
    </div>
    <adminnav>
        <div class="admin_addproduct_deleted" v-if="deleted">
            ...
            <div>
                <div>
                    ...
                    <div>
                        <div>
                            <h3>Product Name</h3>
                            <input :disabled="deleted" :class="deleted ? 'unavailable' : ''" v-model="name" type="text">
                        </div>
                        <div>
                            <h3>Business Description</h3>
                            <div v-html="description" @mouseup="saveSelection(); formatText()" class="admin_addproduct_main_textarea"
                                 :spellcheck="deleted" :contenteditable="!deleted" @keyup="saveSelection()" :class="deleted ? 'unavailable' : ''">
                            </div>
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <div class="admin_addproduct_main_categories">
                        <div v-if="categories.categories" v-for="(el, key) in selectCategories">
                            <h3 @click="chooseCategory = key; !deleted ? togglePopup('admin_addproduct_popup_category') : ''">
                                {{ categories.categories.find(ctg => ctg.id === el.category).name }}
                            </h3>
                            <div @click="chooseCategory = key; !deleted ? togglePopup('admin_addproduct_popup_tag') : ''">
                                {{ el.tag !== -1 ? categories.categories.find(ctg => ctg.id === el.category)
                                .tags.find(tg => tg.id === el.tag).name : "--" }}
                            </div>
                        </div>
                        <div>
                            <h3 @click="chooseCategory = -1; !deleted ? togglePopup('admin_addproduct_popup_category') : ''"
                ...
                <div>
                    ...
                    <div>
                        <div>
                            <h3>Quantity</h3>
                            <input :disabled="deleted" :class="deleted ? 'unavailable' : ''" v-model="stock_quantity" type="number">
                        </div>
                    </div>
                </div>
            </div>
            <div>
                <div>
                    ...
                    <div class="admin_addproduct_main_images">
                        ...
                        <label :style="'cursor: ' + (deleted ? 'unset' : '')"  class="admin_addproduct_main_addimage" @drop="drop" @dragover="ondragover">
                            <input :disabled="deleted" @change="addimg" type="file" accept="image/*" alt="">
                            <div>
                ...
                <div>
                    ...
                    <div>
                        <div>
                            <h3>Price</h3>
                            <input :disabled="deleted" :class="deleted ? 'unavailable' : ''" v-model="price" type="number">
                        </div>
                    </div>
                </div>
                <div class="admin_addproduct_main_buttons">
                    <button @click="discard">Discard</button>
                    <button v-if="!deleted" @click="!deleted ? togglePopup('admin_indexuser_popup_delete') : ''" class="admin_addproduct_main_buttons_delete">Delete product</button>
                    <button @click="sendData" class="admin_addproduct_main_buttons_add">Save product</button>
                </div>
            </div>
        </div>
    </adminnav>

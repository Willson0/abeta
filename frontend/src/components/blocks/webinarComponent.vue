...
<template>
    ...
    :style="'background-color:' + generateSoftRandomColor()">
        <div class="feed_webinar_alt_date" style="color:white">Вебинар &middot; {{webinar.date}}  {{new Date(new Date().setHours(new Date().getHours() + 3)) > new Date(webinar.date.replace(' ', 'T') + "Z") ? 'В записи' : formatDate(webinar.date)}}</div>
    ...
    style="background-color:white; color:black;">
        <div class="feed_webinar_alt_date" style="color:#FF734C">Вебинар &middot; {{webinar.date}} {{new Date(new Date().setHours(new Date().getHours() + 3)) > new Date(webinar.date.replace(' ', 'T') + "Z") ? 'В записи' : formatDate(webinar.date)}}</div>

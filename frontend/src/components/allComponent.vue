...
export default {
    ...
    components: {
        ...
        WebinarComponent,
        ChatBlockComponent,
        AnalyticComponent, ConsultationExpertsComponent, MailComponent, FutureEventsComponent},
    data () {
        return {
            config: config,
            additionAnalytics: [],
    ...
    async mounted () {
        window.addEventListener("scroll", (ev) => {
            ...
            const targetBlock = document.querySelector('.feed .feed_web:nth-last-child(2)');
...
<template>
    <div class="feed">
        <webinar-component v-if="feed.upcoming_events?.[0]" :webinar="feed.upcoming_events?.[0]" />
        <future-events-component :webinars="feed.upcoming_events?.slice(1)"/>
        <old-events-component :webinars="feed.old_events" />
